...
class CLA:
    ...
    def solve(self):
        ...
        while True:
            ...
            if (l_in == None or l_in < 0) and (l_out == None or l_out < 0):
                ...
                self.w.append(np.copy(w))  # store solution self.g.append(g)
                self.f.append(f[:])
                if self.l[-1] == 0: break
    ...
    def initAlgo(self): #initialize algorithm
        #form the structure array
    ...
    '''Prepareing sum of each one (F, FB), nuyF and omegaB'''
    def getMatrices(self,f):
        #slice covarF, covarFB, meanF, meanB, wF, wB
        ...
        return covarF,meanF, covarFB,wB
    ...
    def purgeNumErr(self,tol):
        ...
        while True:
            ...
            for j in range(w.shape[0]):
                if w[j]-self.lB[j]<-tol or w[j]-self.uB[j]>tol: del self.w[i]
                del self.l[i]
                del self.g[i]
                del self.f[i]
                break
    ...
    #1) Compute the local max SR portfolio between any two neighbor turning points w_sr,sr=[],[]
...
if __name__ =='__main__':
    pass
'''Just don't know how to run this file, but ye copying is a type of learning
'''